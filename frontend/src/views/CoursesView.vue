<template>
  <section class="panel">
    ...
    <form v-if="store.isAdmin" @submit.prevent="createCourse" class="form-grid">
      ...
      <button type="submit">{{ $t("courses.add") }}</button>
    </form>

    <table>
      <thead>
        <tr><th>ID</th><th>{{ $t("courses.code") }}</th><th>{{ $t("courses.name") }}</th><th>{{ $t("courses.teacher") }}</th><th>{{ $t("courses.credit") }}</th><th>{{ $t("courses.action") }}</th></tr>
      </thead>
      <tbody>
        <tr v-for="item in store.courses" :key="item.id">
          ...
          <td><button v-if="store.isAdmin" class="danger" @click="store.deleteCourse(item.id)">{{ $t("common.delete") }}</button><span v-else>-</span></td>
        </tr>
      </tbody>
    </table>
...
const search = async () => { store.coursesPage = 1; await safeRun(() => store.fetchCourses()); };
const changePage = async (page) => { store.coursesPage = page; await safeRun(() => store.fetchCourses()); };

const createCourse = async () => {
  if (!form.courseCode || !form.courseName || !form.teacher || form.credit < 1 || form.credit > 8) {
    store.setMessage("error", t("msg.courseFormInvalid"));
    return;
  }
  await safeRun(() => store.addCourse({ ...form }));
  form.courseCode = ""; form.courseName = ""; form.teacher = ""; form.credit = 3;
};

const safeRun = async (fn) => {
  try { store.clearMessage(); await fn(); } catch (e) { store.setMessage("error", e.message); }
};

onMounted(() => safeRun(() => store.fetchCourses()));
</script>
